    ...
    if 'adicionar' in request.GET:
        try:
            ...
            if item_existente:
                ...
                item_existente['subtotal'] = round(
                    item_existente['quantidade'] * preco_venda, 2
                )
            else:
                carrinho.append(
                    {
    ...
    if request.method == 'POST':
        try:
            ...
            if desconto > margem_disponivel:
                messages.error(
                    request, f'Desconto máximo permitido: R$ {margem_disponivel:.2f}'
                )
                return redirect('vendas:venda_rapida')
