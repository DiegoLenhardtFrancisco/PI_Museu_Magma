...
from visitantes.models import Visit, Visitor
...
class DashboardStructureTests(APITestCase):
    ...
    def test_empty_database_returns_zeros(self):
        ...
        self.assertEqual(response.data['visitors']['total_count'], 0)
        self.assertEqual(response.data['products']['low_stock_count'], 0)
...
class DashboardDateFilterTests(APITestCase):
    ...
    def test_only_start_date_is_valid(self):
        response = self.client.get(self.url, {'start_date': '2025-01-01'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIsNone(response.data['period']['end_date'])

    def test_no_date_filter_returns_200(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIsNone(response.data['period']['start_date'])
        self.assertIsNone(response.data['period']['end_date'])
...
class DashboardDataCorrectnessTests(APITestCase):
    ...
    def test_visitor_count_is_correct(self):
        response = self.client.get(self.url)
        self.assertEqual(response.data['visitors']['total_count'], 1)
